...
/// Whether Health Connect will withhold records older than 30 days.
///
/// The 30-day cap and `READ_HEALTH_DATA_HISTORY` shipped together, so a Health
/// Connect that does not offer the history feature does not cap the read
/// either: [isFeatureAvailable] returning false means the full history already
/// came through, and nothing should be requested or reported.
///
/// [requestAccess] is only called when [request] is true and the permission is
/// offered but not yet granted, so a background import never raises a dialog.
@visibleForTesting
Future<bool> historyReadIsCapped({
  required Future<bool> Function() isFeatureAvailable,
  required Future<bool> Function() isGranted,
  required Future<bool> Function() requestAccess,
  required bool request,
}) async {
  if (!await isFeatureAvailable()) {
    return false;
  }
  if (await isGranted()) {
    return false;
...
class HealthConnectService {
  ...
  /// granted yet.
  Future<bool> _isHistoryCapped({required bool request}) => historyReadIsCapped(
    isFeatureAvailable: isHistoryAccessAvailable,
    isGranted: hasHistoryAccess,
    requestAccess: requestHistoryAccess,
    request: request,
  );
  ...
  /// Connect only hands over once the history permission is granted. It is
  /// asked for when [requestHistory] is true.
  Future<HealthConnectImportResult> importMeasurements({
    ...
    // The lock is deliberately held across the permission sheet below: leaving
    // it open lets the resume handler start its own 30-day import while the
    // user is still deciding, and this import would then lose the race and
    // report itself busy.
    _syncing = true;
    try {
  ...
  Future<HealthConnectImportResult> _importMeasurements({
    ...
    try {
      ...
      final bool historyLimited =
          days == null && await _isHistoryCapped(request: requestHistory);
