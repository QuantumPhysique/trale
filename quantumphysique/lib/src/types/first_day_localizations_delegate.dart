...
/// [QPFirstDay] preference.
class QPFirstDayLocalizationsDelegate
    extends LocalizationsDelegate<MaterialLocalizations> {
  /// Creates a delegate that overrides [firstDayOfWeekIndex].
  const QPFirstDayLocalizationsDelegate({required this.firstDay});

  /// The user-chosen first day. [QPFirstDay.Default] means "use locale".
  final QPFirstDay firstDay;

  /// Converts the [QPFirstDay] to the 0-based index used by Flutter.
  int? get _firstDayOfWeekIndex {
    final int? dtWeekday = firstDay.asDateTimeWeekday;
    if (dtWeekday == null) {
      return null;
    }
    return dtWeekday % 7;
  ...
  Future<MaterialLocalizations> load(Locale locale) async {
    ...
    return _QPFirstDayMaterialLocalizations(base, override);
  }

  @override
  bool shouldReload(QPFirstDayLocalizationsDelegate old) =>
      firstDay != old.firstDay;
}

class _QPFirstDayMaterialLocalizations implements MaterialLocalizations {
  const _QPFirstDayMaterialLocalizations(this._base, this._firstDay);

  final MaterialLocalizations _base;
  final int _firstDay;

  @override
  int get firstDayOfWeekIndex => _firstDay;
