...
**Solution**: Rename `date` → `check_in_date` across ALL tables and references  
**Status**: ✅ IMPLEMENTED - Refactor applied; verified in daily_entry_screen.dart
...
- ✅ 5 foreign key constraints (in 3 tables)
- ✅ 2 database triggers
- ✅ 1 query method
- ✅ Generated Drift code (app_database.g.dart)
- ✅ Application queries and inserts
...
CREATE TABLE check_in (
    date    TEXT PRIMARY KEY,   -- ❌ RESERVED KEYWORD
    weight  REAL,
    height  REAL,
    notes   TEXT
);
...
```sql
CREATE TABLE check_in (
    check_in_date TEXT PRIMARY KEY,   -- ✅ ISO-8601 YYYY-MM-DD
    weight        REAL,
    height        REAL,
    notes         TEXT
);
...
**Changes Required:**
- Rename column `date` → `check_in_date` in Drift table class
...
CREATE TABLE workout (
    ...
    FOREIGN KEY (check_in_date)
        REFERENCES check_in(date)          -- ❌ References old column
        ON DELETE CASCADE
);
```
...
CREATE TABLE workout (
    ...
    FOREIGN KEY (check_in_date)
        REFERENCES check_in(check_in_date)  -- ✅ References new column
        ON DELETE CASCADE
);
```
...
CREATE TABLE workout_workout_tag (
    ...
    FOREIGN KEY (check_in_date)
        REFERENCES workout(check_in_date)   -- ✅ Already correct
        ON DELETE CASCADE,
...
WHEN date < date('now','localtime')         -- ❌ Uses 'date' column
BEGIN
...
CREATE TRIGGER IF NOT EXISTS prevent_update_old_checkin
BEFORE UPDATE ON check_in
WHEN check_in_date < date('now','localtime')  -- ✅ Uses 'check_in_date' column
BEGIN
  SELECT RAISE(ABORT, 'check-in is immutable by date');
END;
```
...
if (from < 6) {
  // Migrate from 'date' to 'check_in_date' in check_in table
...
⚠️ **This file is auto-generated by Drift. After making changes to app_database.dart:**
...
### Phase 2: Add Migration Code ✅
1. Add migration logic for version 4 → 5 in `onUpgrade`
...
- [ ] `CheckIns.date` renamed to `CheckIns.checkInDate`
- [ ] All 4 foreign key constraints updated
- [ ] Both database triggers updated (onCreate)
- [ ] Both database triggers updated (onUpgrade)
- [ ] Migration code added for v4 → v5
- [ ] `getCheckInByDate` method updated
- [ ] daily_entry_screen.dart queries updated
- [ ] daily_entry_screen.dart inserts updated
- [ ] homescreen_calendar.dart field access updated
- [ ] `dart run build_runner build` executed successfully
- [ ] No compilation errors
- [ ] Fresh install test passed
- [ ] Migration test passed (v4 → v5)
- [ ] Save check-in test passed
- [ ] Query check-in test passed
...
3. **Database**: Users on v5 would need to:
   ...
   - Or apply reverse migration (v5 → v4)
...
|------|----------|------------|
| Data loss during migration | 🔴 HIGH | Test migration extensively, add transaction rollback |
| Foreign key violations | 🟠 MEDIUM | Disable/re-enable FK during migration, verify constraints |
| Generated code conflicts | 🟡 LOW | Use `--delete-conflicting-outputs` flag |
| Trigger syntax errors | 🟡 LOW | Test triggers individually after migration |
| Backward compatibility | 🔴 HIGH | Schema version bump ensures clean upgrade path |

---

## Related Files to Review

- ✅ `app/lib/core/db/app_database.dart` (primary schema)
- ✅ `app/lib/core/db/app_database.g.dart` (generated)
- ✅ `app/lib/screens/daily_entry_screen.dart` (main usage)
- ✅ `app/lib/pages/homescreen_calendar.dart` (query usage)
- ⚠️ Other potential files with CheckIn usage (search needed)
